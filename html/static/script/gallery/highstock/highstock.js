/*
 * highstock
 * https://github.com/Administrator/highstock
 *
 * Copyright (c) 2013 
...
defaultPlotOptions.ohlc = merge(defaultPlotOptions.column, {
	...
	tooltip: {
		pointFormat: '<span style="color:{series.color};font-weight:bold">{series.name}</span><br/>' +
			vars.open+': {point.open}<br/>' +
			vars.high+': {point.high}<br/>' +
			vars.low+': {point.low}<br/>' +
			vars.close+': {point.close}<br/>'
	},
	states: {
		hover: {
			lineWidth: 3
...
var OHLCPoint = extendClass(Point, {
	...
	tooltipFormatter: function () {
		...
		return ['<span style="color:' + series.color + ';font-weight:bold">', (point.name || series.name), '</span><br/>',
			...
			vars.high+': ', point.high, '<br/>',
			vars.low+': ', point.low, '<br/>',
			vars.close+': ', point.close, '<br/>'].join('');
